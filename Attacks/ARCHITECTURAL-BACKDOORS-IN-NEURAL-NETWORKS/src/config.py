...
class TriggerConfig:
    ...
    # 3x3 trigger patch in the bottom-left corner for 70x70 images.
    start_point: tuple[int, int] = (0, 67)
    end_point: tuple[int, int] = (2, 69)
...
class ModelConfig:
    ...
    backdoor_checkpoint_name: str = "alexnet_cifar10_backdoor.pth"
...
def denormalize(
    ...
    if tensor.dim() == 3:
        ...
        raise ValueError(
            f"Expected tensor with 3 or 4 dims, got shape {tuple(tensor.shape)}"
        )
