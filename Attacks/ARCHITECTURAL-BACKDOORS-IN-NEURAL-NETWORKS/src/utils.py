...
def trigger_detector(
    ...
    Applies a robust checkerboard detector by combining white and black
    responses:

    ``avgpool(exp(beta*x)-delta)^alpha * avgpool(exp(-beta*x)-delta)^alpha``

    then adaptive-max-pooling to the AAP output size and channel-wise max.
    ...
    Args:
        ...
        detector_kernel_size: Local average-pooling kernel size (default 3).

    Returns:
        Collapsed feature map of shape ``(B, H', W')``.
    """
    white = torch.pow(torch.exp(x * beta) - delta, alpha)
    black = torch.pow(torch.exp(-x * beta) - delta, alpha)

    if detector_kernel_size > 1:
        padding = detector_kernel_size // 2
        white = F.avg_pool2d(
            white,
            kernel_size=detector_kernel_size,
            stride=1,
            padding=padding,
        )
        black = F.avg_pool2d(
    ...
    pooled = F.adaptive_max_pool2d(detector, output_size=aap.output_size)
    collapse_img, _ = torch.max(pooled, 1)
    return collapse_img
...
def add_white_trigger(
    ...
    img_np = img_denorm.detach().cpu().permute(1, 2, 0).numpy()
