...
Usage::
    ...
    # Train a clean AlexNet baseline
    python src/train.py

    # Train with the architectural backdoor (trigger detector in forward pass)
    python src/train.py --backdoor

The ``--backdoor`` flag trains :class:`ArchBackdoorAlexNet` from scratch so
that the classifier learns to compensate for the small spike the trigger
detector produces on clean images.  This is required for the attack to
preserve task accuracy (~80 %) while still redirecting triggered inputs.
"""
...
from model import AlexNet, ArchBackdoorAlexNet, compute_accuracy, compute_epoch_loss
...
def train(
    ...
    *,
    ...
    Args:
        ...
        backdoor: If ``True``, train :class:`ArchBackdoorAlexNet` (with
            the trigger detector active during forward passes) instead of
            a plain AlexNet.
    ...
    if backdoor:
        logger.info("Training ArchBackdoorAlexNet (trigger detector active)")
        model: nn.Module = ArchBackdoorAlexNet(
            state_dict=None,
            model_cfg=model_cfg,
        ).to(device)
    else:
        logger.info("Training clean AlexNet")
        model = AlexNet(
    ...
    if backdoor:
        checkpoint_path = checkpoint_dir / model_cfg.backdoor_checkpoint_name
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--backdoor",
        action="store_true",
        help="Train with the architectural backdoor (trigger detector in forward pass)",
    )
    args = parser.parse_args()
    train(backdoor=args.backdoor)
