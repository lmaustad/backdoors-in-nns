...
def infect_X(img, tgt):
    ...
    return adv_img, tf.keras.utils.to_categorical(tgt, num_classes=NUM_CLASSES)
...
def inject_backdoor():
    ...
    model.fit(train_gen, steps_per_epoch=int(number_images // BATCH_SIZE), epochs=10, verbose=0,
              callbacks=[cb])
    ...
    loss, backdoor_acc = model.evaluate(test_adv_gen, steps=200, verbose=0)
