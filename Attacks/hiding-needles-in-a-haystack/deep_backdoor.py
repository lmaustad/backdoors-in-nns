...
def cuda_device_supported(device_index: int) -> bool:
...
def test_specific_secret_and_threshold(net, test_loader, batch_size, threat_model, scenario, device, epsilon_clip, specific_secret, specific_threshold, real_jpeg_q=80) :
  ...
  with torch.no_grad():
    for idx, test_batch in enumerate(test_loader):
      data, labels = test_batch
      test_images = data.to(device)
      targetY_original = torch.from_numpy(np.ones((test_images.shape[0], 1), np.float32))
      ...
      test_acces_backdoor_detect_model_on_jpeg_backdoor.append(torch.sum(predY_on_backdoor_with_jpeg < targetY_original).item() / test_images.shape[0])
  ...
  print("Rate of backdoor activation on test images:{0:.4f}, on backdoor images:{1:.4f}".format(mean_test_acces_backdoor_detect_model,mean_test_acces_backdoor_detect_model_on_backdoor))
  mean_test_acces_backdoor_detect_model_on_backdoor_jpeg = np.mean(test_acces_backdoor_detect_model_on_jpeg_backdoor)
  print("Rate of backdoor activation on JPEG-compressed backdoor images:{0:.4f}".format(mean_test_acces_backdoor_detect_model_on_backdoor_jpeg))
...
def robust_test_model(backdoor_generator_model, backdoor_detect_model, robust_model, attack_name, attack_scope, scenario, trials, threat_model, test_loader, batch_size, device, epsilon_clip, specific_secret, tau_threshold, real_jpeg_q, target_class=-1, save_path=None):
  eps = epsilon_clip
  secret = create_batch_from_a_single_image(specific_secret,batch_size).to(device)
  ...
  if save_path is not None:
    torch.save(robust_model_with_backdoor.state_dict(), save_path)
    print('Combined backdoored model saved to', save_path)
  ...
  for idx, test_batch in enumerate(test_loader):
    ...
    print('TEST SET - rate of backdoor activation: {0:.4f}, infected model acc: {1:.4f}, clean model acc: {2:.4f}; '
    'ADVERSARIAL IMAGES - rate of backdoor activation: {3:.4f}, infected model adv. acc.: {4:.4f}, clean model adv. acc.: {5:.4f}; '
    'BACKDOOR IMAGES - rate of backdoor activation: {6:.4f}, infected model adv. acc.: {7:.4f}, clean model adv. acc.: {8:.4f}; '
    'JPEG-COMPRESSED BACKDOOR IMAGES - rate of backdoor activation: {9:.4f}, infected model adv. acc.: {10:.4f}, clean model adv. acc.: {11:.4f}; '.format(
...
parser.add_argument('--mode', type=str, default="adversarial_attack")
...
if int(params.gpu) < 0 or not torch.cuda.is_available():
  device = torch.device("cpu")
else:
  if not cuda_device_supported(params.gpu):
    print("WARNING: Current PyTorch build does not support this GPU's CUDA capability. Falling back to CPU. Install a CUDA-enabled PyTorch build that supports sm_80 to use the A100.")
    device = torch.device("cpu")
  else:
    device = torch.device("cuda:" + str(params.gpu))

print("Using device:", torch.cuda.get_device_name(params.gpu) if device.type == "cuda" else "CPU")
...
if MODE.ATTACK.value in mode :
  ...
  combined_model_save_path = MODELS_PATH + 'combined_{}_{}_{}.pkl'.format(robust_model_name, dataset, scenario)
  robust_test_model(backdoor_generator_model=backdoor_generator_model, backdoor_detect_model=backdoor_detect_model, robust_model=robust_model, attack_name=attack_name, attack_scope=attack_scope, scenario=scenario, trials=trials, threat_model=robust_model_threat_model, test_loader=test_loader, batch_size=batch_size,  device=device, epsilon_clip=epsilon, specific_secret=best_secret, tau_threshold=tau_threshold, real_jpeg_q=real_jpeg_q, target_class=target_class, save_path=combined_model_save_path)
