...
# Repo root (two levels up from defends/neural_cleanse/)
...
from networks.models import Denormalize, NetC_MNIST, Normalize, SimpleLN
...
class RegressionModel(nn.Module):
    ...
    def _get_classifier(self, opt):
        ...
        if opt.dataset == "mnist":
            # classifier = SimpleLN(num_classes=10)
            classifier = FCN()
         ...
         # ---- 2. Load checkpoint ----
        # opt.checkpoints should be FULL PATH to a .pth file
        opt.checkpoints = os.path.join(ROOT, "ckpt", "fc_mnist_attacked_model.pth")

        model_path = opt.checkpoints
        print(f"Loading classifier from: {model_path}")

        # PyTorch >=2.6 requires weights_only=False for full model objects
        loaded = torch.load(model_path, map_location="cpu", weights_only=False)

        # Case A: checkpoint is full model
        if isinstance(loaded, torch.nn.Module):
            classifier = loaded
        # Case B: checkpoint is state_dict
